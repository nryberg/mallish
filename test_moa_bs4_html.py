...
output_csv = "./test_listings_moa.csv"
...
def process_file(file_path, csv_writer):
    ...
    listingContents = soup.select("tr > td > table > tbody > tr > td") #  id="listingContent")

    target_element = soup.find('a', href=lambda href: href and 'about_moa_tenant_profile_objectname' in href)
    ...
    sections = parent.select("table")

    for section in sections:

        # STORES or RESTAURANTS
        if section.find('strong'):
            section_name = section.find('strong').getText()
            print(section_name)

        rows  = section.select("tbody > tr", recursive=False)
